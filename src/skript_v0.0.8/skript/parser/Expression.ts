...
export class Expression {
	...
	constructor(name: string, ...pattern:string[]) {
		...
		for (const p of pattern) {
			this._pattern.push(new Pattern(p));
		}
	}
...
class PatternParser {
	...
	public static parse(pattern: string): PatternElement {
		pattern = pattern.trim().replace(/\s+/, 's');
...
export class Pattern {
	...
	constructor(pattern: string) {
		this._legacy = pattern;
		this._elements = PatternParser.parse(pattern);
...
/** 한가지 선택해야하는 패턴 */
class ChoiceGroup extends ChoiceElement {

}

/** 생략가능한 패턴 */
class OptionalGroup extends ChoiceElement {
